...
public class RecursoDAO {
    ...
    public static void guardar(Recurso recurso) {
        String sql = "INSERT INTO recursos (titulo, tipo, enlace, ruta_archivo, curso_id) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = ConexionDB.getConexion();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, recurso.getTitulo());
            ps.setString(2, recurso.getTipo().name());
            ps.setString(3, recurso.getEnlace());
            ps.setString(4, recurso.getRutaArchivo());
            ps.setInt(5, recurso.getCursoId());
            ps.executeUpdate();
        ...
        try (Connection conn = ConexionDB.getConexion();
            ...
            while (rs.next()) {
                ...
                if (tipo == Recurso.TipoRecurso.PDF) {
                    r = new Recurso(rs.getString("titulo"), rs.getString("ruta_archivo"), cursoId);
                } else {
                    r = new Recurso(rs.getString("titulo"), tipo, rs.getString("enlace"), cursoId);
                }
